...
it("limits the number of results", async () => {
  const res1 = await GET("/items/search?q=smart&limit=1");
...
it("implements cursor-based pagination", async () => {
  ...
  const res2 = await GET(
    `/items/search?q=smart&limit=${limit}&cursor=${body1.next_cursor}&threshold=${body1.next_threshold}`,
  );
...
it("returns 400 if query does not exist", async () => {
  const res = await GET("/items/search");
...
it("returns 400 if query is less than 1 character long", async () => {
  const res = await GET("/items/search?q=");
  const body = await res.json();
...
it("returns 400 if query is more than 100 characters long", async () => {
  const res = await GET(`/items/search?q=${"a".repeat(101)}`);
  const body = await res.json();
...
it("returns 400 if limit is not an integer", async () => {
  const res = await GET("/items/search?q=smart&limit=1.5");
  const body = await res.json();
...
it("returns 400 if limit is less than 1", async () => {
  const res = await GET("/items/search?q=smart&limit=0");
  const body = await res.json();
...
it("returns 400 if limit is greater than 100", async () => {
  const res = await GET("/items/search?q=smart&limit=101");
  const body = await res.json();
...
it("returns 400 if cursor is invalid", async () => {
  const res = await GET("/items/search?q=smart&cursor=foo");
  const body = await res.json();
...
it("returns 400 if threshold is not a number", async () => {
  const res = await GET("/items/search?q=smart&threshold=foo");
  const body = await res.json();
...
it("returns 400 if threshold is less than 0", async () => {
  const res = await GET("/items/search?q=smart&threshold=-1");
  const body = await res.json();
