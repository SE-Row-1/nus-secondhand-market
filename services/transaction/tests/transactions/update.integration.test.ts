...
import { PATCH } from "../test-utils/request";
...
it("returns 401 if not logged in", async () => {
  const res = await PATCH(`/transactions/${crypto.randomUUID()}`, {
    action: "complete",
  });
...
it("returns 409 if transaction to complete is already completed", async () => {
  const transaction: Transaction = {
    id: crypto.randomUUID(),
    item: {
      id: crypto.randomUUID(),
...
it("returns 409 if transaction to complete is already cancelled", async () => {
  const transaction: Transaction = {
    id: crypto.randomUUID(),
    item: {
      id: crypto.randomUUID(),
...
it("returns 409 if transaction to cancel is already completed", async () => {
  const transaction: Transaction = {
    id: crypto.randomUUID(),
    item: {
      id: crypto.randomUUID(),
...
it("returns 409 if transaction to cancel is already cancelled", async () => {
  const transaction: Transaction = {
    id: crypto.randomUUID(),
    item: {
      id: crypto.randomUUID(),
